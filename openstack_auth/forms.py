    ...
    def __init__(self, *args, **kwargs):
        ...
        if getattr(settings,
            ...
            self.fields.keyOrder = ['domain', 'username', 'password', 'region']
                        ...
                        False):
            self.client = client.Client(token="password",
                                        endpoint=getattr(settings, 'OPENSTACK_KEYSTONE_FEDERATED_URL'))
...
class FederatedProjectForm(forms.Form):
    '''
        Form for choosing a project to scope to after federated authentication
    '''

    projects = forms.ChoiceField(label=_('Project'), required=True)

    def __init__(self, *args, **kwargs):
        super(FederatedProjectForm, self).__init__(*args, **kwargs)
        CHOICES = ()
        for project in args[0]:
            self.fields['projects'].choices.insert(0,(json.dumps(project), project['id']))
